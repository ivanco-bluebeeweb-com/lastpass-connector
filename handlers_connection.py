...
async def connect_lastpass_connector(ctx, params: ConnectParams) -> ActionResult:
...
async def disconnect_lastpass_connector(ctx, params: ConnectionIdParams) -> ActionResult:
    conns = await _load_conns(ctx)
    if not conns:
