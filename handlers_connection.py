...
async def connect_lastpass_connector(params: ConnectParams, ctx) -> ActionResult:
    ...
    return ActionResult.ok(rec, summary=f"Connected LastPass Enterprise ({rec['label']}).")
...
async def disconnect_lastpass_connector(params: ConnectionIdParams, ctx) -> ActionResult:
    ...
    if params.connection_id:
        ...
        conns.clear()
    await _save_conns(ctx, conns)
    return ActionResult.ok({"success": True, "message": "Disconnected successfully."}, summary="Disconnected LastPass Enterprise connection.")
