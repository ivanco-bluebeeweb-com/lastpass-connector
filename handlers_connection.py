"""Connection lifecycle for LastPass Connector."""
...
@chat.function(
    "connect_lastpass",
    "Connect LastPass account via credentials.",
    action_type="write",
    chain_callable=True,
    event="lastpass-connector.connect_lastpass",
    effects=["create:connection"],
    data_model=ConnectParams
)
async def connect_lastpass(params: ConnectParams, ctx) -> ActionResult[ConnectionRecord]:
    """Connect LastPass Connector."""
    client = LastPassClient(api_key=params.api_key, base_url=params.base_url)
    await client.verify_auth()
    conns = await _load_connections(ctx)
    cid = f"conn_{uuid.uuid4().hex[:8]}"
    record = {
        "id": cid,
        "label": params.label or "LastPass Account",
...
@chat.function(
    "list_connections",
    "List connected LastPass accounts.",
    action_type="read",
    chain_callable=True,
    data_model=NoParams
)
...
@chat.function(
    "disconnect_lastpass",
    "Disconnect LastPass account.",
    action_type="write",
    chain_callable=True,
    event="lastpass-connector.disconnect_lastpass",
    effects=["delete:connection"],
    data_model=ConnectionIdParams
)
async def disconnect_lastpass(params: ConnectionIdParams, ctx) -> ActionResult[DeleteResult]:
    conns = await _load_connections(ctx)
